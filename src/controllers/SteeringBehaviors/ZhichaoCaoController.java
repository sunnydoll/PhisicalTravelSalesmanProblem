...
public class ZhichaoCaoController extends Controller
{
	/**
	...
	private int wallAvoid = 0;
	private int wallAvoidNow = 0;
	private Vector2d wallPoint = new Vector2d(0, 0);
	 ...
	 */
	public ZhichaoCaoController(Game a_gameCopy, long a_timeDue) {
		// Init the graph.
...
//		 System.out.println("DIST: " + distanceToColl);
		...
		if (isThereLineOfSight) {
		...
		else if (!isThereLineOfSight && distanceToColl < 30 && wallAvoid == 0) {
			wallAvoid = 1;
		}
		else if (!isThereLineOfSight && distanceToColl < 30 && wallAvoid == 1) {
			return Controller.ACTION_THR_LEFT;
		}
		else if (!isThereLineOfSight && distanceToColl < 30 && wallAvoid == 0 && wallAvoidNow == 0) {
			//Detect the available point to get rid of obstacles
			double x = 0;
			double y = 0;
			boolean line = true;
			Vector2d vtemp = new Vector2d(0, 0);
			vtemp.set(wallPoint.x + 10, wallPoint.y);
			line = a_gameCopy.getMap().LineOfSight(a_gameCopy.getShip().s, vtemp);
			if(!a_gameCopy.getMap().isObstacle((int)wallPoint.x + 10, (int)wallPoint.y) && line) {
				x = wallPoint.x + 10;
				y = wallPoint.y;
				wallPoint.set(x, y);
			}
			...
			vtemp.set(wallPoint.x, wallPoint.y + 10);
			line = a_gameCopy.getMap().LineOfSight(a_gameCopy.getShip().s, vtemp);
			if(!a_gameCopy.getMap().isObstacle((int)wallPoint.x, (int)wallPoint.y + 10) && line) {
				x = wallPoint.x;
				y = wallPoint.y + 10;
				wallPoint.set(x, y);
			}
			...
			vtemp.set(wallPoint.x + 10, wallPoint.y + 10);
			line = a_gameCopy.getMap().LineOfSight(a_gameCopy.getShip().s, vtemp);
			if(!a_gameCopy.getMap().isObstacle((int)wallPoint.x + 10, (int)wallPoint.y + 10) && line) {
				x = wallPoint.x + 10;
				y = wallPoint.y + 10;
				wallPoint.set(x, y);
			}
			...
			line = a_gameCopy.getMap().LineOfSight(a_gameCopy.getShip().s, wallPoint);
		}
    	...
    	Vector2d curDirCopy = curDir.copy();
    	curDirCopy.normalise();
    	if(wallAvoid == 1 && wallAvoidNow == 0) {
//    		targPoi = curPos + 20 * curDir.normalise();
    		wallPoint.set(curPos.x + 20 * curDirCopy.x, curPos.y + 20 * curDirCopy.y);
    		wallAvoidNow = 1;
    	}
    	System.out.println(curPos.x);
    	Vector2d targetPoint = wallPoint.copy();
    	targetPoint.subtract(curPos);
    	double distance = targetPoint.mag();	//D
    	if(targetPoint.mag() < 0.8) {
    		wallAvoid = 0;
    		wallAvoidNow = 0;
    	}
