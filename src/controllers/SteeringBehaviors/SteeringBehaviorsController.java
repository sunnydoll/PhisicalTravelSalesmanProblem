 ...
 * Date: 20/12/11
 */
public class SteeringBehaviorsController extends Controller
{
    /**
     ...
     */
    public SteeringBehaviorsController(Game a_gameCopy, long a_timeDue)
    {
        //Init the graph.
...
//        }
        ...
        {        	
        	...
        	int turn = 0;
        	int maxAcc = 1;
        	Game gameSeek = a_gameCopy.getCopy();
        	Node nextNode = getNextNode();
        	Vector2d nextNodeV = new Vector2d(nextNode.x(),nextNode.y());
        	Vector2d currentPos = gameSeek.getShip().s;
        	Vector2d currentSpd = gameSeek.getShip().v;
        	Vector2d potentialDirection = gameSeek.getShip().d;
        	nextNodeV.subtract(currentPos);  //D
            nextNodeV.normalise();  //ND
            //Vector2d nextNodeVA = new Vector2d(nextNodeV.x * 0.025, nextNodeV.y * 0.025);  //A        	
        	double dot = potentialDirection.dot(nextNodeV);
            ...
            if(distance > 80 || a_gameCopy.getStepsLeft() < 50) {
                thrust = 1;
            }
            else if(distance < 30) {
            	thrust = 0;
...
//        		thrust = 0;
//        	}
//        	double spd = java.lang.Math.sqrt(currentSpd.x*currentSpd.x + currentSpd.y*currentSpd.y);
//        	if(spd < 0.5) {
//        		thrust = 1;
//        	}
//        	else if(spd >= 0.5) {
//        		thrust = 0;
//        	}
        	double alpha = java.lang.Math.acos(dot);
//        	double radDist = (potentialDirection.y/potentialDirection.x)*(currentPos.x - nextNodeV.x) - (currentPos.y - nextNodeV.y);
        	double b = currentPos.y - (potentialDirection.y/potentialDirection.x)*currentPos.x;
        	double radDist;
			if(potentialDirection.x != 0)
        	radDist = (potentialDirection.y/potentialDirection.x)*(currentPos.x - nextNodeV.x) - (currentPos.y - nextNodeV.y) + b;
        	else radDist = - (currentPos.y - nextNodeV.y) + b;
//        	double y = currentPos.y+nextNode.y();
//            double x = currentPos.x+nextNode.y();
//            double radDist =- (y-currentPos.y) + potentialDirection.y/potentialDirection.x*(x-currentPos.x);
        	
//        	if(radDist < 5 && radDist > -5) {
//        		turn = 0;
//        	}
        	if(radDist > 0) {
        		turn = 1;
        	...
        	if(thrust == 0 && turn == 0) {
        		startAction = Controller.ACTION_NO_FRONT;
        	}
        	else if(thrust == 0 && turn == 1) {
        		startAction = Controller.ACTION_NO_LEFT;
        	}
        	else if(thrust == 0 && turn == -1) {
        		startAction = Controller.ACTION_NO_RIGHT;
        	}
        	else if(thrust == 1 && turn == 0) {
        		startAction = Controller.ACTION_THR_FRONT;
        	}
        	else if(thrust == 1 && turn == 1) {
        		startAction = Controller.ACTION_THR_LEFT;
        	}
        	else if(thrust == 1 && turn == -1) {
        		startAction = Controller.ACTION_THR_RIGHT;
        	}
        	
//        	if(gameSeek.getShip().getCollLastStep()) {
...
//        	System.out.println(spd);
        	...
        	System.out.println(thrust + "  " + turn);
    ...
    public void motorControl(Game a_gameCopy, long a_timeDue) {
    	int maxAcc = 2;
    	Game gameSeek = a_gameCopy.getCopy();
    	Node nextNode = getNextNode();
    	Vector2d nextNodeV = new Vector2d(nextNode.x(),nextNode.y());
    	Vector2d currentPos = gameSeek.getShip().s;
    	Vector2d distance = nextNodeV.subtract(currentPos);  //D
    	double n = java.lang.Math.sqrt(distance.x*distance.x + distance.y*distance.y);
    	//distance.normalise();    	
