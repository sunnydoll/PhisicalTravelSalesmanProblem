...
public class JianyuController extends Controller {
	 ...
	 */
	public JianyuController(Game a_gameCopy, long a_timeDue) {
		// Init the graph.
		...
		for (Waypoint way : a_gameCopy.getWaypoints()) {
			m_collectNodes.put(way,
					m_graph.getClosestNodeTo(way.s.x, way.s.y, true));
		}
	...
	public int getAction(Game a_gameCopy, long a_timeDue) {
		...
		// Wanna know the distance to the closest obstacle ahead of the ship?
		// Try:
		// double distanceToColl =
		// a_gameCopy.getMap().distanceToCollision(a_gameCopy.getShip().s,
		// a_gameCopy.getShip().d, 1000);
		...
		if (isThereLineOfSight) {
			return myNoObstacleAction(a_gameCopy);
		}
		...
		if (m_pathToClosest != null) // We should have a path...
		{
			...
			for (int action = startAction; action < Controller.NUM_ACTIONS; ++action) {
				...
				// Get the next node to go to, from the path to the closest
				// waypoint
				Node nextNode = getNextNode();
				Vector2d nextNodeV = new Vector2d(nextNode.x(), nextNode.y());
				nextNodeV.subtract(nextPosition);
				nextNodeV.normalise(); // This is a unit vector from my position
										// pointing towards the next node to go
										// to.
				double dot = potentialDirection.dot(nextNodeV); // Dot product
																// between this
																// vector and
																// where the
																// ship is
				...
				// Get the distance to the next node in the tree and update the
				// total distance until the closest waypoint
				double dist = nextNode.euclideanDistanceTo(nextPosition.x,
						nextPosition.y);
				double totalDistance = m_pathToClosest.m_cost + dist;

				// System.out.format("Action: %d, total distance: %.3f, distance to node: %.3f, dot: %.3f\n",action,
				// totalDistance, dist, dot);
				...
				else if ((int) totalDistance == (int) minDistance
						&& dot > bestDot) {
					minDistance = totalDistance;
					bestAction = action;
					bestDot = dot;
	...
	private Node getNextNode() {
		...
		if (n0.euclideanDistanceTo(destination) < n1
				.euclideanDistanceTo(destination))
			return n0;
		else
			return n1;
	}
	...
	private void calculateClosestWaypoint(Game a_gameCopy) {
		...
		for (Waypoint way : a_gameCopy.getWaypoints()) {
			...
			{
				double fx = way.s.x - a_gameCopy.getShip().s.x, fy = way.s.y
						- a_gameCopy.getShip().s.y;
	...
	private int myNoObstacleAction(Game a_gameCopy) {
		Ship me = a_gameCopy.getShip();
		int choice = Controller.ACTION_NO_FRONT;

		Vector2d target = m_closestWaypoint.s.copy();

		target.subtract(me.s);
		double distance = target.mag();
		target.normalise();
		...
		int flag = 0;
		double angle = getAngle(me.d) - getAngle(target);
		Vector2d v = me.v;
		double vangle = 0;
		double velocity = v.mag();
		if (velocity != 0) {
			v.normalise();

			vangle = getAngle(v) - getAngle(me.d);
		}
		if (target.dot(me.d) > 0.99)
			flag = 0;
		else if (angle > 0 && angle <= Math.PI || angle < -Math.PI) {
			flag = 1;
		} else if (angle > Math.PI || angle < 0 && angle >= -Math.PI)
			flag = 2;
		double bigAngle = Math.PI / 2;
		if (velocity > maxspeed || vangle == bigAngle
				|| vangle == -bigAngle) {
			choice = Controller.ACTION_NO_FRONT + flag;
		} else {
			choice = Controller.ACTION_THR_FRONT + flag;
		}
		return choice;
	}

	private double getAngle(Vector2d dot) {
		if (dot.y > 0)
			return Math.acos(dot.x);
		else if (dot.y < 0)
			return -Math.acos(dot.x);
		else if (dot.x > 0)
			return 0;
	...
	private int manageStraightTravel(Game a_gameCopy) {
		...
		// Depending on the time left and the distance to the waypoint, we
		// established the maximum speed.
		// (going full speed could make the ship to overshoot the waypoint...
		// that's the reason of this method!).
		double maxSpeed = 0.4;
		...
		if (notFacingWaypoint || a_gameCopy.getShip().v.mag() > maxSpeed) {
			// We should not risk to throttle. Let's rotate in place to face the
			// waypoint better.
			Game forThisAction;
			double bestDot = -2;
			for (int i = Controller.ACTION_NO_FRONT; i <= Controller.ACTION_NO_RIGHT; ++i) {
				// Select the action that maximises my dot product with the
				// target (aka. makes the ship face the target better).
	...
	public void paint(Graphics2D a_gr) {
		...
		if (pathToClosest != null)
			for (int i = 0; i < pathToClosest.m_points.size() - 1; ++i) {
				...
				Node nextNode = m_graph.getNode(pathToClosest.m_points
						.get(i + 1));
				a_gr.drawLine(thisNode.x(), thisNode.y(), nextNode.x(),
						nextNode.y());
			}
	}
