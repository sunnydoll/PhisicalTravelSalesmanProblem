...
public class ZhichaoCaoControllerBack extends Controller
{
	/**
	 ...
	 */
	public ZhichaoCaoControllerBack(Game a_gameCopy, long a_timeDue) {
		// Init the graph.
...
//		 System.out.println("DIST: " + distanceToColl);
		...
		if (isThereLineOfSight) {
			wallAvoid = 0;
			wallAvoidNow = 0;
			wallPoint.subtract(0, 0);
			return motorControl(a_gameCopy);
		}
		else if (!isThereLineOfSight && distanceToColl < 30 && wallAvoid == 0) {
			wallAvoid = 1;
		}
		else if (!isThereLineOfSight && distanceToColl < 30 && wallAvoid == 1 && wallAvoidNow == 1) {
			double x = 0;
			double y = 0;
			boolean line = true;
...
//			wallAvoid = 0;
//			int step = motorControl(a_gameCopy);
//			if(step == 2 || step == 5) {
